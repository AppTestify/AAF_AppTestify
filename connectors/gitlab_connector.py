...
class GitLabConnector(BaseConnector):
    ...
    async def _fetch_live(self, ctx: FetchContext) -> dict[str, Any]:
        ...
        project = getattr(self.settings, "gitlab_project_id", None) or ""
        ...
        out: dict[str, Any] = {"simulated": False, "project_id": project}
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                mrs = await client.get(
                    f"{base_url}/api/v4/projects/{project}/merge_requests",
                    headers=headers,
                    params={"state": "opened", "per_page": 20},
                )
                out["merge_requests"] = mrs.json() if mrs.status_code == 200 else []
                pipes = await client.get(
                    f"{base_url}/api/v4/projects/{project}/pipelines",
                    headers=headers,
                    params={"per_page": 15},
                )
