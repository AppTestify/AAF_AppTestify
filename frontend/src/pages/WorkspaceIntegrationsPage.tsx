...
import { ConnectorHealthBar } from "../components/charts/ConnectorHealthBar";
...
export function WorkspaceIntegrationsPage({ tenantSlug, canManage }: WorkspaceIntegrationsPageProps) {
  ...
  const connectorHealthDonut = useMemo(() => {
    const health = summary?.connector_health ?? [];
    let healthy = 0;
    let failing = 0;
    let unknown = 0;
    let disabled = 0;
    for (const c of health) {
      if (!c.enabled) disabled += 1;
      else if (c.last_validation_ok === true) healthy += 1;
      else if (c.last_validation_ok === false) failing += 1;
      else unknown += 1;
    }
    return [
  ...
  const load = async () => {
    const [connectorRows, providerRows, lifecycleSummary] = await Promise.all([
  ...
  return (
    <WorkspacePageShell
      variant="operational"
      title="Integrations & Telemetry"
      subtitle="Connector/provider health, validation, and runtime readiness"
    >
      {displayError ? (
      ...
      <div className="card integrations-chart-card">
        <div className="integrations-chart-grid">
          <DonutChart
            data={connectorHealthDonut}
            title="Connector validation health"
            emptyMessage="Connector health appears once telemetry is available."
          />
          <ConnectorHealthBar connectors={summary?.connector_health ?? []} title="Validation timeline" />
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="settings-grid">
            ...
            <div className="metric">
              ...
              <div
                className={`value ${obs?.slo_burn_rate.state === "critical" ? "bad" : obs?.slo_burn_rate.state === "warning" ? "warn" : "good"}`}
              >
