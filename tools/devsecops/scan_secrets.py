...
async def _direct_scan_secrets(ctx: ToolContext) -> ToolResult:
...
async def scan_secrets(ctx: ToolContext) -> ToolResult:
    ...
    return await run_with_transport(
        ctx,
        agileops_tool="scan_secrets",
        mcp_tool="list_secret_scanning_alerts",
        direct_fn=_direct_scan_secrets,
    )
